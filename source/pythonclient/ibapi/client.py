...
class EClient(object):
    ...
    def run(self):
        """This is the function that opens and create the persistent websocket connection. It runs the message loop."""
            ...
            while self.isConnected() or not self.msg_queue.empty():
                try:
                    try:
                        ...
                        logger.debug("fields %s", fields)
                        self.decoder.interpret(fields)  # This line interprets the msg returned by IB API server
                        self.msgLoopRec()
    ...
    def placeOrder(self, orderId: OrderId, contract: Contract, order: Order):
        ...
        if self.serverVersion() < MIN_SERVER_VER_SOFT_DOLLAR_TIER:
            if order.softDollarTier.event_name or order.softDollarTier.val:
                self.wrapper.error(orderId, UPDATE_TWS.code(), UPDATE_TWS.msg() +
        ...
        try:
            ...
            if self.serverVersion() >= MIN_SERVER_VER_SOFT_DOLLAR_TIER:
                flds += [make_field(order.softDollarTier.event_name),
    ...
    def reqContractDetails(self, reqId: int, contract: Contract):
        ...
        reqId: int - The ID of the data request. Ensures that responses are
            make_field() attached to requests if several requests are in process.
        ...
        try:
            ...
            # send req mkt data msg
            flds = []
            flds += [make_field(OUT.REQ_CONTRACT_DATA),  # REQ_CONTRACT_DATA = 9
                     make_field(VERSION)]
    ...
    def reqHistoricalData(self, reqId: TickerId, contract: Contract, endDateTime: str,
        ...
        barSizeSetting:str - Specifies the size of the bars that will be returned (within IB/TWS listimits).
            ...
            1 hour
            1 day
        whatToShow:str - Determines the nature of data being extracted. Valid values include:
            TRADES
            MIDPOINT
            BID
            ASK
        ...
        formatDate: int - Determines the date format applied to returned bars. valid values include:
    ...
    def reqFundamentalData(self, reqId: TickerId, contract: Contract,
        ...
        reqFundamentalData() can handle con_id specified in the Contract object,
